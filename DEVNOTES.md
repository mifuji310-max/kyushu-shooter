...
| v0.9.0 | 【feature/mode7-groundブランチ】擬似3D 案B=Mode7風の地面: 背景をPhaser Planeに貼り奥へ倒す(tilt 1.35rad・上=遠くが圧縮)。テクスチャは縦2枚重ねJPG(640x2768, `img/kumamoto_bg3_plane.jpg`)でuvScrollにより継ぎ目なくループ。スクロールは`uvScroll(0, dv)`をデルタ補正で毎フレーム設定(**頂点UV直接変更はMeshキャッシュに反映されないため不可**、`ignoreDirtyCache=true`も併用)。調整値はBALANCE.mode7*。旧tileSprite背景と_skyは廃止。検証はキャンバスをtoDataURL→JPEG保存→目視の手法を確立(preview_screenshotがタイムアウトする環境向け) |
