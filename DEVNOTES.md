...
| v0.13.0 | 武器を直交3軸＋乗算に再設計(SHOT): 団子=連射速度/拡散=奇数way(1→3→5→7・正面キープ)/大玉=サイズ威力貫通(乗算)/ほっぺ=ハート化+破裂(乗算)。団子と拡散の機能重複を解消。総合Lvで弾色進化(シアン→青→紫→金)・最大でADD発光の尾・取得時に自機脈動+光リング。弾は白テクスチャ(p_orb/p_heart/p_comet)をtint/scaleで合成。アイテムアイコンを役割が伝わるよう描き直し。ボスを全体0.9倍(135/216/256/180)。ボス④を強化(スパイラル5ray+逆回転扇+狙い2=9発/回・間隔780ms)。HP係数を後半増(1.3/1.9/2.7/2.3) |
| v0.12.0 | ボス4段階を個性化(BOSS_PHASES): 表示サイズを両目間隔基準で150→240→285→200(成長→凝縮)・当たり判定も表示連動・HP係数[1.2,1.7,2.3,1.9](④<③)。動き=①遅い単調/②重く小刻み/③中速+突進/④ランダムワープ。攻撃=①狙い扇/②下向き8way+狙い/③下向き12way+3way+火の玉/④スパイラル+高速狙い。**全方位リング撤廃→下向き扇(0.08π-0.92π)**で上向き弾を廃止。弾色をe_orb(白)のtintで多色化(ボス=赤/紫/橙/シアン、恐竜=緑/陣太鼓=琥珀)。差し替えボス画像4枚を透過抜き |
| v0.11.0 | Mode7遠近背景を**デフォルト化**(実機確認済み)。?m7=gridは格子デバッグ、?m7=flatで旧平面に戻せる避難ハッチを用意 |
| v0.10.1 | Mode7ストリップのガタツキ対策: 帯を40本(15px)から120本(5px)に細分化。帯境界の拡大率ジャンプが1/3になり、縦線(道路等)の階段状の段差が大幅に減少。描画面積は不変でコスト増はドローコール分のみ |
